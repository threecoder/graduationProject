<template>
	<view class="vote-container">
		...
		<view class="oper">
			<button class="button-small" v-if="item.status == '可报名'" @click="join">报名</button>
			<button class="button-small" v-if="item.status == '未支付'" @click="pay">支付</button>
			<button class="button-small" v-if="item.status == '已支付'" @click="seatNum">座位</button>
...
export default {
	...
	methods: {
		async join() {
			try {
				let res = await activityApi.studentJoinActivties(this.item.id);
				toast('报名成功');
				this.$emit('refresh');
			} catch (error) {
				toast(error.message);
		...
		async seatNum() {
			try {
				let res = await activityApi.getSeatNum(this.item.id);
